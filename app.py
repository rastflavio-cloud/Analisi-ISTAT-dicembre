...
with st.expander(f"📝 ANALISI DETTAGLIATA - {mese}", expanded=True):
    ...
    dis_str_t = f"<span style='color:black; font-weight:bold;'>{d['v_dis_t']:+} mila</span>"
    ...
    * Incidenza Occ: **{d['inc_occ_c']}%** | Incidenza Ina: **{d['inc_ina_c']}%**
    <br>
    **VARIAZIONE ANNUALE:** Occ {color_val(d['v_occ_t'])} | Dis {dis_str_t} | Ina {color_val(d['v_ina_t'])}
    * Incidenza Occ: **{d['inc_occ_t']}%** | Incidenza Ina: **{d['inc_ina_t']}%**
